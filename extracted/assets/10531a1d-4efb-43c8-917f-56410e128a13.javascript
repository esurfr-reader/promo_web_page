        ...
        <div style={{background:'rgba(255,255,255,.92)',backdropFilter:'blur(8px)',borderRadius:18,padding:'14px 16px',boxShadow:'0 8px 24px rgba(14,42,77,.18)'}}>
          ...
          <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',marginTop:4}}>
            <div style={{fontFamily:'Fraunces, serif',fontSize:36,fontWeight:700,letterSpacing:'-.03em',color:'#0e2a4d',lineHeight:1}}>
              3.8<span style={{fontSize:18,color:'#90a0b6'}}>/5</span>
            </div>
...
function Flow1Map() {
  return (
    <div style={{position:'absolute',inset:0}}>
      <img src={window.__resources.beach_right_hand} style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover'}}/>
      {/* Me icon */}
      <img src={window.__resources.me_surfer} style={{position:'absolute',left:'42%',top:'52%',width:48,height:48,filter:'drop-shadow(0 4px 6px rgba(0,0,0,.4))',transform:'translate(-50%,-85%)'}}/>
      {/* wave arrow */}
...
function Flow3Review() {
  return (
    <div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,#0e2a4d,#143b6e)',padding:'22px',display:'flex',flexDirection:'column',justifyContent:'center',gap:8,color:'white'}}>
      ...
      {[
        ['Paddle out',5],['Wave selection',4],['Take-off',4],['Bottom turn',3],['Surf execution',4],
      ].map(([k,v],i)=>(
        <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}>
          ...
          <div style={{display:'flex',gap:2}}>
            {[1,2,3,4,5].map(s=>(
              <svg key={s} width="14" height="14" viewBox="0 0 24 24" fill={s<=v?'#F08A2A':'rgba(255,255,255,.18)'}><path d="M12 2l2.9 6.9 7.4.6-5.6 4.9 1.7 7.3L12 17.8 5.6 21.7l1.7-7.3L1.7 9.5l7.4-.6z"/></svg>
            ))}
          </div>
        </div>
      ))}
      <div style={{marginTop:10,padding:'10px 12px',background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.18)',borderRadius:12,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <span style={{fontFamily:'Fraunces, serif',fontWeight:700,fontSize:24,letterSpacing:'-.02em'}}>4.0 <span style={{color:'#F4C77E',fontSize:14}}>★</span></span>
      </div>
    </div>
  );
}
...
const BREAKS = [
  { id: 'right_hand', label: 'Right-hand point', img: window.__resources.beach_right_hand },
  { id: 'open',       label: 'Open beach',       img: window.__resources.beach_open },
  { id: 'reef',       label: 'Beach reef',       img: window.__resources.beach_reef },
  { id: 'rivermouth', label: 'Rivermouth',       img: window.__resources.beach_river_mouth },
];

const TRAY = [
  { id:'me',      img: window.__resources.me_surfer,           label:'Me' },
  { id:'house',   img: window.__resources.icon_house,          label:'House' },
  { id:'tree',    img: window.__resources.icon_tree,           label:'Tree' },
  { id:'rocks',   img: window.__resources.icon_rocks,          label:'Rocks' },
  { id:'tower',   img: window.__resources.icon_lifesaver_tower,label:'Lifesaver tower' },
  { id:'flag',    img: window.__resources.icon_lifesaver_flag, label:'Flag' },
  { id:'track',   img: window.__resources.icon_walking_track,  label:'Walking track' },
  { id:'park',    img: window.__resources.icon_carpark,        label:'Car park' },
  { id:'building',img: window.__resources.icon_building,       label:'Building' },
];
...
function ReviewDemo() {
  const [ratings, setRatings] = useState([5, 4, 4, 3, 4, 4, 5]);
  const avg = useMemo(()=> {
    ...
    <div className="review-card">
      ...
      <div className="summary">
        <div className="left">
          ...
          <div className="big">{avg.toFixed(1)} <span style={{color:'#F08A2A'}}>★</span></div>
        </div>
        <div className="ramp" aria-hidden="true">
          {[1,2,3,4,5].map(i => <span key={i} className={i<=filledRamp?'on':''}/>)}
        </div>
      </div>
